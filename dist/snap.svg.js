...
var mina = (function (eve) {
    ...
    mina.elastic = function (amp, per, n) {
        ...
        let f = per / _2PI * (Math.asin(1 / a) || 0);
...
var Snap_ia = (function(root) {
      ...
      function Snap(w, h) {
        if (w) {
          if (w.nodeType || w instanceof jQuery) {
            return wrap(w);
          }
      ...
      var has = 'hasOwnProperty',
          ...
          tSrtToRemove = /atrix|ranslate|cale|otate|kewX|kewY|\(|\)/ig,
          idgen = 0,
...
// Copied from http://bl.ocks.org/mbostock/8027637
      Snap.closestPoint = function(path, x, y) {
        ...
        for (var scan, scanLength = 0, scanDistance; scanLength <=
...
// Colour
      ...
      var toHex = function(color) {
            ...
            toHex = cacher(function(color) {
              ...
              i.style.color = red;
              i.style.color = color;
              var out = glob.doc.defaultView.getComputedStyle(i, E).
...
// Transformations
      ...
      var parseTransformString = Snap.parseTransformString = function(TString) {
        ...
        if (!data.length) {
          Str(TString).
              replace(tSrtToRemove, '').
              replace(tCommand, function(a, b, c) {
                var params = [],
                    name = b.toLowerCase();
                c.replace(pathValues, function(a, b) {
                  b && params.push(+b);
...
// Hub garbage collector every 10s
      const gurbage_collect = function() {
        if (glob.win.requestIdleCallback) {
          return function() {
            glob.win.requestIdleCallback(function() {
                  for (var key in hub) if (hub[has](key)) {
                    var el = hub[key],
                        node = el.node;
                    if (el.type !== 'svg' && !node.ownerSVGElement || el.type ===
                        'svg' &&
                        (!node.parentNode || 'ownerSVGElement' in node.parentNode &&
      ...
      function Element(el) {
        ...
        this.type = el.tagName || el.nodeName
            || ((el instanceof jQuery) ? 'jquery' : null);
      ...
      function sanitize(svg) {
        var script_filter = /<script[\s\S]*\/script>/gmi;
...
// default
      ...
      eve.on('snap.util.attr', function(value) {
        ...
        if (cssAttr[has](css)) {
          ...
          if (geomAttr[att]) this.c_hull = undefined;
        }
      });
      (function(proto) {
      }(Paper.prototype));
      ...
      Snap.load = function(
        ...
        if (data) {
          ...
          Snap.ajax(url, function(req) {
            var f = Snap.parse(req.responseText, filter_event);
            scope ? callback.call(scope, f) : callback(f);
...
Snap_ia.plugin(function(Snap, Element, Paper, glob, Fragment, eve) {
  ...
  elproto.getPoints = function(use_local_transform) {
    let result = [];
    let rx, ry, matrix;
    switch (this.type) {
      case 'polyline':
      ...
      case 'symbol':
        ...
        for (let i = 0, l = children.length; i < l; ++i) {
          const pts = children[i].getPoints(true);
          pts && (result = [...result, ...pts]);
        }
        break;
      ...
      case 'use': //todo:
        ...
        matrix = Snap.matrix().
            add(this.getLocalMatrix().
                translate(this.attr('x') || 0, this.attr('y') || 0));
  ...
  elproto.getCHull = function(with_transform) {

    if (with_transform) {
      let m = this.getLocalMatrix();
      if (!m.isIdentity()) {
        let chull = this.getCHull();
        return chull.map((p) => m.apply(p));
      }
    }

    if (this.c_hull) return this.c_hull;
  ...
  elproto.getUseTarget = function() {
    ...
    if (this.use_target) {
      ...
      if (href) {
        ...
        return this.use_target = Snap._.hub[elementId] ||
  ...
  elproto.getBBox = function(settings) {
    ...
    let isWithoutTransform, cache_bbox, include_clip_path, relative_parent,
        relative_coord, matrix, approx = true;
    ...
    if (this.type === 'tspan' || this.type === 'text') {
      ...
      //make a clone put it in the root of the svg
      function compute_with_clone(text_el) {
        let temp_clone = text_el.node.cloneNode(true);
        temp_clone.removeAttribute('transform');
        temp_clone = text_el.paper.node.appendChild(temp_clone);
        const rect = temp_clone.getBoundingClientRect();
        temp_clone.remove();
        return rect;
      }

      clientRect = compute_with_clone(this);
    ...
    while (el.type === 'use') {
      if (!isWithoutTransform) {
        matrix_for_x_y_attrs = matrix_for_x_y_attrs.add(el.getLocalMatrix().
            translate(el.attr('x') || 0, el.attr('y') || 0));
      }
    ...
    if (el.type === 'g') {
      el.matrix = el.getLocalMatrix(true);
      ...
      let bboxes = children.map(function(elm) {
        ...
        const box = elm_cl.getBBox(isWithoutTransform);
        if (+cache_bbox === 1) {
          elm.attr('bbox',
              box.x + ' ' + box.y + ' ' + box.width + ' ' + box.height +
              ((clip_path) ? ' cp' : ''));
  ...
  function clearParentCHull(el) {
    let parent = el.parent();
    while (parent && parent.type !== 'svg') {
      parent.c_hull && (parent.c_hull = undefined);
    ...
    if (this.node) {
      if (this.type == 'linearGradient' || this.type == 'radialGradient') {
        ...
        try {
          ...
          var dom_partner = this.data('dom_partner');
          if (dom_partner) {
            for (let i = 0, l = dom_partner.length, dom; i < l; ++i) {
              dom = dom_partner[i];
              if (dom.css) {
                dom.css({transform: this.node.getAttribute('transform')});
              } else if (glob.jQuery) {
                dom = jQuery(dom);
                dom.css({transform: this.node.getAttribute('transform')});
              }
          ...
          var element_partner = this.data('element_partner');
          if (element_partner && element_partner.length) {
            for (let i = 0, l = element_partner.length; i < l; ++i) {
              $(element_partner[i].node, {transform: this.matrix});
            }
          }
  ...
  elproto.append = elproto.add = function(el) {
    ...
    if (el) {
      ...
      // }
      if (el.setPaper && el.paper !== this.paper) {
  ...
  elproto.removeUses = function() {
    ...
    if (this.type === 'use') {
      ...
      if (!this.use_target) {
        ...
        if (href) {
          const elementId = href.substring(href.indexOf('#') + 1);
          this.use_target = Snap._.hub[elementId] ||
              wrap(this.node.ownerDocument.getElementById(elementId));
          if (!this.use_target) {
